...
### 3.5 Optional env key checks (Supabase-enabled VPS stack)
...
### 4.2 Trigger Render backend deploy
...
### 4.3 Deploy frontend to Vercel production
...
### 4.4 Health checks (Vercel + Render stack)
...
### 5.1 Safer rollback (keep history)
...
### 5.2 Re-run deploy path

Run section `3` for VPS stack or section `4` for Vercel+Render stack.

### 5.3 Emergency VPS-only rollback (no git history change)
